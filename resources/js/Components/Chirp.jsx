...
export default function Chirp({ chirp }) {
    const { auth } = usePage().props; //props is an object that contains all of the props that were passed to the page component.
            ...
            <div className="flex-1">
                <div className="flex justify-between items-center">
                    ...
                    {chirp.user.id === auth.user.id &&
                        <Dropdown>
                            <Dropdown.Trigger> {/*Dropdown.Trigger is a button that opens the dropdown menu}*/}
                                <button>
                            ...
                            <Dropdown.Content> {/*Dropdown.Content is the dropdown menu itself*/}
                                <button className="block w-full px-4 py-2 text-left text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:bg-gray-100 transition duration-150 ease-in-out" onClick={() => setEditing(true)}>
                                    Edit
                                </button>
                                {/*Knop toegevoegd voor het deleten*/}
                                <Dropdown.Link as="button" href={route('chirps.destroy', chirp.id)} method="delete">
                                    Delete
                                </Dropdown.Link> {/*Dropdown.Link is a link that closes the dropdown menu when clicked*/}
                            </Dropdown.Content>
                        </Dropdown>
                    }
                </div>
